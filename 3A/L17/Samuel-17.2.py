...
def save_pic_to_db(conn, table_name, stu_name, diary, pic_file):
    """ 把图片字节码存入数据库 """
    cur = conn.cursor()  # 获得数据库的操作游标
    ...
    #使用?占位符，是安全的sql语句
    cur.execute(sql, (stu_name, pic_file, diary, content))
    conn.commit()
...
if __name__ == '__main__':
    ...
    #pip install virtualencreate_table(conn, table_name)
    save_pic_to_db(conn, table_name, 'Crinkle Leaf', 'It is a plant!', 'Crinkle Leaf.jpeg')
    save_pic_to_db(conn, table_name, 'Graptoveria', 'abc', 'Graptoveria.jpeg')
