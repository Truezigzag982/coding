...
def main():
    ...
    layout = [
        ...
        [sg.Text('日期:'),
         sg.Input(do_not_clear=False, size=(10, 1), key='_DATE_'),
         sg.CalendarButton('选择日期', target=('_DATE_'), format='%Y-%m-%d', no_titlebar=False)],

        [sg.Text('事由:'),
         sg.Input(do_not_clear=False, size=(26, 1), key='_CONTENT_')],
    ...
    while True:
        ...
        if event == '保存':
            ...
            new_record_list = get_record(filename, date, content, amount, memo)
            print('new record:', new_record_list)
